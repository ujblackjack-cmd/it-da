...
const convertBackendNotification = (
  ...
  const frontendType = convertNotificationType(
...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  fetchNotifications: async (userId?: number) => {
    ...
    try {
      ...
      const backendNotifications = response.notifications.map(
        convertBackendNotification,
      );

      // 기존 실시간 알림(웹소켓)은 유지하고 백엔드 알림만 교체
      const realtimeNotifications = get().notifications.filter(
        (n) => !n.id.startsWith("backend-"),
      );
      ...
      set({
        ...
        unreadCount:
          ...
          realtimeNotifications.filter((n) => n.isUnread).length,
        loading: false,
      });
  ...
  addFollowNotification: (data) => {
    ...
    const messageText =
      data.message ||
      `${data.fromUsername}${typeMap[notificationType] || "님이 회원님을 팔로우했습니다."}`;
    const now = new Date();
