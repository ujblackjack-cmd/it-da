...
import { User } from "../../types/user.types"; // 유저 타입 정의 가정
...
const ChatMemberList: React.FC<Props> = ({ members, onFollow, onReport }) => {
    return (
        <div className="flex flex-col gap-3">
            {members.map((member) => (
                <div
                    key={member.userId} // ✅ id -> userId로 변경
                    className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors"
                >
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold">
                        {(member.nickname || member.username || "익")[0]}
                    </div>
                    ...
                    <div className="flex gap-2">
                        {member.role !== "ME" && (
                            <>
                                <button
                                    onClick={() => onFollow(member.userId)} // ✅ id -> userId
                                ...
                                <button
                                    onClick={() => onReport(member.userId, member.nickname || member.username)} // ✅ 필드명 수정
