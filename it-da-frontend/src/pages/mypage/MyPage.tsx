...
const MyPage: React.FC = () => {
    ...
    const handleProfileUpdate = useCallback(
        (update: ProfileUpdate) => {
            ...
            if (update.type === "PROFILE_UPDATE") {
                ...
                if (update.field === "participationCount" && update.value !== undefined) {
    ...
    const handleToggleFollowUser = async (targetUserId: number) => {
        ...
        try {
            ...
            setFollowUsers((prev) =>
                prev.map((u) => (u.userId === targetUserId ? { ...u, isFollowing: !u.isFollowing } : u)),
            );
        } catch (err: unknown) {
            ...
            if (errorMessage.includes("이미 팔로우")) {
                setFollowUsers((prev) =>
                    prev.map((u) => (u.userId === targetUserId ? { ...u, isFollowing: true } : u)),
                );
            } else {
    ...
    const handleUserClick = (userId: number) => {
        ...
        navigate(`/${followUsers.find((u) => u.userId === userId)?.email.split("@")[0]}`);
    };
    ...
    const handleOpenMeetingReviews = (meetingId: number, meetingTitle: string) => {
                ...
                <div className="mypage-header-wrapper">
                    <div className="mypage-header-content">
                        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                            <button className="mypage-back-btn" type="button" onClick={() => navigate("/")}>←</button>
                        ...
                        <div className="mypage-header-actions">
                            <button className="mypage-icon-btn" type="button" onClick={() => setIsNotificationOpen(!isNotificationOpen)}>
                                🔔
                                {unreadCount > 0 && <span className="mypage-badge">{unreadCount}</span>}
                            </button>
