...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({
  ...
  const handleAcceptFollow = async (
    ...
    try {
      await apiClient.post(
        `/api/users/${user.userId}/follow-request/${notification.fromUserId}/accept`,
      );
  ...
  const handleRejectFollow = async (
    ...
    try {
      await apiClient.post(
        `/api/users/${user.userId}/follow-request/${notification.fromUserId}/reject`,
      );
      removeNotification(notification.id);
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    // ✅ 모임 관련 알림
      ...
      navigate(`/meetings/${notification.relatedId}`);
    }
    // ✅ 후기 관련 알림
    else if (
      ["review", "review_request"].includes(notification.type) &&
      notification.relatedId
    ) {
      navigate(`/meetings/${notification.relatedId}/review`);
    }
    // ✅ 팔로우 관련 알림 - 프로필로 이동
  ...
  const getProfileInfo = (notification: Notification) => {
    ...
    return {
      ...
      name:
        notification.fromUsername || notification.senderName || "알 수 없음",
    };
  };
